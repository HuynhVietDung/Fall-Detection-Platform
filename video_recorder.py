...
class CameraCapture:
    ...
    def start_recording(self) -> None:
        ...
        while True:
            for i, vid_cap in enumerate(self.video_captures):
                out = self._start_video_writer(video_id, i)  # Pass camera index
                self._record_video(vid_cap, out, self.frames_per_video)
                out.release()
                video_id += 1

            # Introduce overlap delay
            time.sleep(self.video_duration - self.overlap_duration)
    ...
    def _start_video_writer(self, video_id: int, camera_id: int):

        start_timestamp = datetime.now().strftime("%S-%M-%H-%d-%m-%y")
        end_time = datetime.now() + timedelta(seconds=self.video_duration)
        end_timestamp = end_time.strftime("%S-%M-%H-%d-%m-%y")

        filename = os.path.join(self.output_path, f"{start_timestamp}_{end_timestamp}_{camera_id}_{video_id}.mp4")
        print(filename)  # TODO: test
